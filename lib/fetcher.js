...
module.exports = heroku => {
  function * attachment (app, db) {
    // this is kind of gross, but cannot do a generator in a catch
    // so basically this makes it easier to handle the 422 & 404
    function * matchesHelper (app, db) {
      ...
      .then(attached => ({matches: [attached], err: null}))
      .catch(function (err) {
    ...
    if (!matches) {
      ...
      matches = attachments.filter(attachment => config[pgUtil.getUrl(attachment.config_vars)] === config[db])
    }
