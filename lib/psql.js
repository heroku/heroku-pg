...
function env (db) {
  return Object.assign({}, process.env, {
    ...
    PGSSLMODE: db.host === 'localhost' ? 'prefer' : 'require',
    PGUSER: db.user || '',
...
function buildTunnelPromise(db, dbTunnelConfig) {
...
function buildPsqlPromise(query, dbEnv) {
...
function buildPsqlInteractivePromise(dbEnv, prompt) {
  const {spawn} = require('child_process')
  return new Promise((resolve, reject) => {
    let psql = spawn('psql',
...
function * exec (db, query) {
  ...
  const command = tunnelPromise.then((tnl) => {
    let psqlPromise = buildPsqlPromise(query, configs.dbEnv)
    return psqlPromise
  })
