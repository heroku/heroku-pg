...
function * exec (db, query, timeout = 20000) {
  ...
  const tunnelPromise = sshTunnel(db, configs.dbTunnelConfig, timeout)
  const command = tunnelPromise.then((tnl) => {
    let psqlPromise = execPsql(query, configs.dbEnv, timeout)
    return psqlPromise
  }).catch((error) => {
    cli.error(error)
  })
...
function * interactive (db) {
  ...
  const tunnelPromise = sshTunnel(db, configs.dbTunnelConfig)
  const command = tunnelPromise.then((tnl) => {
    let psqlPromise = psqlInteractive(configs.dbEnv, prompt)
    return psqlPromise
  }).catch((error) => {
    cli.error(error)
    process.exit(1)
